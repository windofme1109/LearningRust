// 宏（macro）
// 术语宏 (macro)其实是 Rust 中的某一组相关功能的集合称谓
// 其中包括使用 macro_rules! 构造的声明宏(declarative macro)及另外3种过程宏 (procedural macro):
// 1. 用于结构体或枚举的自定义#[derive]宏，它可以指定随 derive 属性自动添加的代码
// 2. 用于为任意条目添加自定义属性的属性宏
...
// 我们可以使用单一参数调用 println! ("hello")，也可以使用两个参数调用 println! ("hello {}", name)
...
// 宏的缺点
// 1. 宏的定义要比函数定义复杂得多，因为你需要编写的是用于生成 Rust 代码的 Rust 代码
...
// 从核心形式上来讲，声明宏要求你编写出类似于 match 表达式的东西
// match 表达式是一种接收其他表达式的控制结构，它会将表达式的结果值与模式进行比较，并在匹配成功时执行对应分支中的代码
// 类似地，宏也会将输入的值与带有相关执行代码的模式进行比较: 此处的值是传递给宏的字面 Rust 源代码
// 而此处的模式则是可以用来匹配这些源代码的结构。当某个模式匹配成功时，该分支下的代码就会被用来替换传入宏的代码
// 所有的这一切都会发生在编译时期

// 为了定义一个宏，我们需要用到 macro_rules!

// 使用 marco_rules! 宏定义一个简版的 vec!

// 代码在 lib-macro_rules.rs 中
